...
$timevar = "SELECT time FROM sent ORDER BY time DESC LIMIT 0,1";
...
if (mysqli_num_rows($timevar_exec) > 0) {
    ...
    if ($emails) {
        ...
        foreach ($emails as $email_number) {
            ...
            $msg_ref = "sent_email_{$email_number}.txt";
            $check_old_email_exist = "SELECT id FROM sent WHERE message='$msg_ref'";
            ...
            if (mysqli_num_rows($check_old_email_exist_exec) == 0) {
                file_put_contents("sent_email_{$email_number}.txt", $message);
                $sql = "INSERT INTO `gmail`.`sent` (`username` , `to` , `subject` , `message` , `time` )VALUES ( '$username', '$to' , '$subject' , '$msg_ref' , '$time')";
...
header("Location:index.php?type=sent");
?>   
